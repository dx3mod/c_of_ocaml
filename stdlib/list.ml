...
let nth_exn l n =
  match nth l n with None -> invalid_arg "List.nth" | Some a -> a
...
let rec concat = function [] -> [] | l :: r -> l @ concat r
...
let filter_map l ~f =
  let rec aux acc = function
    | [] -> rev acc
    | x :: xs -> (
        match f x with None -> aux acc xs | Some y -> aux (y :: acc) xs)
  in
  aux [] l
...
let reduce l ~f =
  match l with [] -> None | x :: xs -> Some (fold_left ~f x xs)
...
let sort_and_group l ~compare =
  ...
  let rec group acc current_key current_vals = function
    ...
    | (k, v) :: rest ->
        if compare k current_key = 0 then
          group acc current_key (v :: current_vals) rest
        else
          let acc' =
            match current_vals with
            | [] -> acc
          ...
          group acc' k [ v ] rest
  in
  match sorted with [] -> [] | (k, v) :: rest -> group [] k [ v ] rest
