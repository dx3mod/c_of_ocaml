...
(** [rev_map2 ~f l1 l2] gives the same result as {!rev}[ (]{!map2}[ ~f l1 l2)],
    but is more efficient. *)

val fold_left2 :
  f:('acc -> 'a -> 'b -> 'acc) -> 'acc -> 'a list -> 'b list -> 'acc
...
val fold_right2 :
  f:('a -> 'b -> 'acc -> 'acc) -> 'a list -> 'b list -> 'acc -> 'acc
(** [fold_right2 ~f [a1; ...; an] [b1; ...; bn] init] is
    [f a1 b1 (f a2 b2 (... (f an bn init) ...))].
    @raise Invalid_argument
      if the two lists are determined to have different lengths. Not
      tail-recursive. *)

val filter_map : 'a list -> f:('a -> 'b option) -> 'b list

val partition_map :
  'a list -> f:('a -> [ `First of 'b | `Second of 'c ]) -> 'b list * 'c list
...
val sort_and_group :
  ('k * 'v) list -> compare:('k -> 'k -> int) -> ('k * 'v list) list
