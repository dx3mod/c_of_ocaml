...
let get s i =
  if i < 0 || i >= length s then raise (Invalid_argument "String.get")
  else unsafe_get s i
...
let lsplit2 s ~on =
  ...
  let rec find i =
    if i >= len then None
    else if Char.equal (unsafe_get s i) on then Some i
    else find (i + 1)
  in
  match find 0 with
  | None -> None
...
let strip s =
  ...
  let rec find_start i =
    if i >= len then i
    else if Char.is_whitespace (unsafe_get s i) then find_start (i + 1)
    else i
  in
  let rec find_end i =
    if i <= 0 then 0
    else if Char.is_whitespace (unsafe_get s (i - 1)) then find_end (i - 1)
    else i
  in
...
let equal s1 s2 =
  ...
  if len1 <> len2 then false
    ...
    let rec loop i =
      if i >= len1 then true
      else if not (Char.equal (unsafe_get s1 i) (unsafe_get s2 i)) then false
      else loop (i + 1)
    in
    loop 0
