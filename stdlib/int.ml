...
let of_string s =
  ...
  if len = 0 then failwith "Int.of_string"
  else
    let start, sign =
      if Char.equal (String.get s 0) '-' then (1, -1)
      else if Char.equal (String.get s 0) '+' then (1, 1)
      else (0, 1)
    in
    let rec loop i acc =
      if i >= len then acc
        ...
        if Char.is_digit c then loop (i + 1) ((acc * 10) + (Char.code c - 48))
