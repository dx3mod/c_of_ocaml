...
let test_no_raise () =
  let result = try 42 with _ -> 0 in
  print_int result

(* Test 2: try/with that raises *)
let test_simple_raise () =
  let result = try raise Stop with Stop -> 99 in
  print_int result

(* Test 3: Nested try/with - inner catches *)
let test_nested_inner () =
  let result = try try raise Stop with Stop -> 77 with _ -> 0 in
  print_int result
...
let test_nested_outer () =
  let result = try try raise MyExn with Stop -> 0 with MyExn -> 88 in
  print_int result
...
let test_deep_raise () =
  let result = try deep_raise 100 with Stop -> 66 in
  print_int result
...
let test_exn_value () =
  let result = try raise (WithInt 123) with WithInt n -> n in
  print_int result
...
let test_multiple_handlers () =
  let f exn = try raise exn with A -> 1 | B -> 2 | C -> 3 in
...
let test_catch_any () =
  let result = try raise (WithInt 999) with _ -> 22 in
  print_int result
