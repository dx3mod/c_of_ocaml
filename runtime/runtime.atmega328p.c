...
#define Header_set_mark(h) ((h) | ((uintnat)1 << 8))
...
static trap_frame trap_stack[TRAP_STACK_SIZE];
static trap_frame *trap_sp = trap_stack;
static value exn_value = 0;

static uint8_t led_states[32] = {0};
...
static value *alloc(uintnat size, uchar tag) {
  ...
  if (hp + words > heap + HEAP_SIZE) {
    gc();
    if (hp + words > heap + HEAP_SIZE) {
      fatal_error("Out of heap memory");
    }
  }
  block = hp;
  hp += words;
  *block = Make_header(size, tag);
  memset(block + 1, 1, size * sizeof(value));
  return block;
}
...
value caml_putc(value c) {
  hal_putchar((char)Int_val(c));
  return Val_unit;
}

value caml_getc(value unit) {
  (void)unit;
  return Val_int((uchar)hal_getchar());
}

value caml_gpio_mode(value pin, value mode) {
  hal_gpio_mode((uint8_t)Int_val(pin), (uint8_t)Int_val(mode));
  return Val_unit;
}

value caml_led_on(value pin) {
  hal_gpio_write((uint8_t)Int_val(pin), 1);
  return Val_unit;
}

value caml_led_off(value pin) {
  hal_gpio_write((uint8_t)Int_val(pin), 0);
  return Val_unit;
}

value caml_led_toggle(value pin) {
  uint8_t p = (uint8_t)Int_val(pin);
  if (p < 32) {
    led_states[p] = !led_states[p];
    hal_gpio_write(p, led_states[p]);
  }
  return Val_unit;
}

value caml_delay_ms(value ms) {
  hal_delay_ms((uint32_t)Int_val(ms));
  return Val_unit;
}
...
value caml_exit(value code) {
  (void)code;
  fatal_error("Program called exit()");
  return Val_unit;
}
